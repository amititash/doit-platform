...
class Profile extends Component {co
    ...
    async componentDidMount () {
        ...
        if(user){
            ...
            if(user.email !== 'engineering@startiq.org' ){
                this.props.location.history.push('/home');
            }   
            let ideas = [];
            try {
                ...
                let adminResponse = await coreModuleinstance.get('/admin/allData');
                ...
                let adminData = adminResponse.data.info;
                this.setState({
                ...
                ideas.forEach( (element, index) => {
                    table.push(
                        <tr>
                            <td><h3>{element.ideaName || `No Idea Name`}</h3> <span></span></td>
                            <td><a href={`mailto:${element.ideaOwner}`}>{element.ideaOwner}</a> </td>
                            <td>{ (element.progress && element.progress == 5) ? <Link to={`/report?idea=${element.ideaNameSlug}&email=${element.ideaOwner}`}>Show Report</Link> : <a target="_blank" href={`https://startiq-duke.slack.com/archives/DP26RHX1D`}>Research this idea</a>}</td>
                        </tr>
                    )
                })
                let card = (
