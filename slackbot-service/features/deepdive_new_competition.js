...
module.exports = function(controller) {
    const DIALOG_ID = "deepdive_new_competition_dialog";
    ...
    convo.before("default", async (convo, bot) => {
        ...
        convo.setVar("companyDescriptionRatedThreeOrAbove" , null);
        convo.setVar("maxCompanyRating", 0);
        convo.setVar("incrementOriginalIndex", true);
        convo.setVar("originalIndex", 0);
        convo.setVar("displayedCompaniesId" , []);
        convo.setVar("companyCounter", 0);
        return await convo.gotoThread("es_query_thread");

    })

    convo.before("es_query_thread", async (convo, bot) => {
        console.log("Text to be queried on elastic search : ", convo.vars.text_to_query || convo.vars.currentKo.ideaDescription);
        convo.vars.maxCompanyRating = 0;
        try {
            convo.vars.allSimilarCompanies = await elasticSearchService.search(convo.vars.text_to_query || convo.vars.currentKo.ideaDescription , convo.vars.displayedCompaniesId);
        ...
        console.log("first index" , convo.vars.companyIndex );
        ...
        return await convo.gotoThread("companies_found_thread");
    })


    convo.addMessage({
        text : "We found the following competitors. On a scale of 1 to 5, how relevant are they to what are you starting?"
    },"companies_found_thread");


    convo.addAction("ask_es_listed_company_relevance_thread", "companies_found_thread");
    ...
    convo.before("ask_es_listed_company_relevance_thread", async (convo, bot) => {
        ...
        convo.vars.companyCounter ++ ;
        if(convo.vars.displayedCompaniesId.indexOf(company._id) !== -1){
            console.log("Non unique company id in dislayed companies id array");
        }
        convo.vars.displayedCompaniesId.push(company._id);
        ...
        convo.setVar("companyBlock",[
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text" : `${convo.vars.companyCounter}. <${company._source.domain}|${company._source.company_name}>\n${companyDescription}`
                }
            },
            {
                "type": "actions",
        ...
        },
        {
            ...
            handler : async (res, convo, bot) => {
                ...
                if(isNaN(score) || score<1 || score > 5){
                    ...
                    if(score >= convo.vars.maxCompanyRating){
                        convo.vars.maxCompanyRating = score;
                        convo.vars.text_to_query = convo.vars.allSimilarCompanies[convo.vars.companyIndex]._source.description;
                    }
                    ...
                    if(convo.vars.incrementOriginalIndex) {
                        convo.vars.originalIndex ++;
                    }
                    console.log("new original index" , convo.vars.originalIndex);
    ...
    [
        ...
        },
        {
            pattern : "yes",
            handler : async (res, convo, bot) => {
                if(convo.vars.maxCompanyRating >= 3){
                    convo.vars.companyIndex = 0;
                    convo.vars.incrementOriginalIndex = false;
                }
                else {
                    convo.vars.text_to_query = convo.vars.currentKo.ideaDescription;
                    // convo.vars.companyIndex = convo.vars.originalIndex;
                    convo.vars.companyIndex = 0;
                    convo.vars.incrementOriginalIndex = true;
                }
                return await convo.gotoThread("es_query_thread");
            }
        },
    ...
    convo.addMessage({
        text : "Let's now head over to the *Target customers* section to know more about the target customer segement."
    },"progress_card_redirect_thread");
    ...
    controller.on('message, direct_message', async(bot, message) => {
        ...
        if(message.text === "xoxo") {
            Logger.log({
                level : "info",
                message : "START of conversation : new_competition_intent",
                metadata : {
                    convo : true,
